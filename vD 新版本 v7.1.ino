...
ActuatorStatus actuator1 = {false, false, 0, 50, true};   // 左側 - 測試中
ActuatorStatus actuator2 = {false, false, 0, 50, false};  // 右側 - 暫停
...
// 馬達方向追蹤 (新增)
bool lastMotorDirectionWasUp = false;  // 追蹤上次馬達移動方向
bool motorIsRunning = false;           // 馬達是否正在運轉
...
const float SPEED_UP_THRESHOLD = 15.0;      // 15 km/h 上升
const float SPEED_DOWN_THRESHOLD = 12.0;    // 12 km/h 下降（使用遲滯避免頻繁切換）
...
bool readLimitSwitch(byte pin, ActuatorStatus &actuator, bool isUp) {
  ...
  bool reading = (digitalRead(pin) == LOW);  // LOW = 磁鐵靠近 = 觸發
  bool &current = isUp ? actuator.upLimitPressed : actuator.downLimitPressed;
  
  // 檢測變化
  if (reading != current) {
    // 開始去抖動計時
    if (actuator.lastChangeTime == 0) {
    ...
    if (millis() - actuator.lastChangeTime > actuator.debounceTime) {
      // 邊緣檢測: 從未觸發到觸發
      bool newTrigger = (reading && !current);
      
      current = reading;
      actuator.lastChangeTime = 0;  // 重置計時器
      
      return newTrigger;  // 返回是否為新觸發
    }
  } else {
    // 訊號穩定，重置計時器
    actuator.lastChangeTime = 0;
  }
...
void updateLimitSwitches() {
  ...
  if(leftUpTriggered || leftDownTriggered) {
    ...
    // 新增：顯示觸發資訊
    if(leftUpTriggered) {
      Serial.println(F(">>> [磁上] 觸發！致動器到達上限位"));
    }
    if(leftDownTriggered) {
      Serial.println(F(">>> [磁下] 觸發！致動器到達下限位"));
    }
  }
}
...
void checkHardwareLimits() {
  ...
  if (anyAtBottom) {
    analogWrite(PWM_PIN, 0);
    motorIsRunning = false;
    if (currentState == STATE_FALLING || currentState == STATE_INIT) {
...
void controlPWM(bool direction_up, int pwm_value) {
  ...
  motorIsRunning = true;
  lastMotorDirectionWasUp = direction_up;
}

void stopAllPWM() {
  analogWrite(PWM_PIN, 0);
  motorIsRunning = false;
  Serial.print(F("[馬達] 停止 - 最後方向: "));
  Serial.println(lastMotorDirectionWasUp ? F("上升") : F("下降"));
}

void moveUp() {
  if (!anyAtTop) {
    controlPWM(true, pwmSpeedUp);
    Serial.println(F("[馬達] 啟動 - 目標上升"));
  }
}
    ...
    case PRIORITY_AUTO:
      if(currentSpeed >= SPEED_UP_THRESHOLD && speedHighCount >= SPEED_STABLE_COUNT) {
        ...
        if(!wheelDown) {
          wheelDown = true;
          ctrlSrc = 3;
          Serial.println(F("自動: 下降 (速度 < 12km/h)"));
        }
      }
      // 12-15 km/h 灰色地帶：維持當前狀態
      break;
  }
}
...
void handleStateMachine() {
  ...
  switch (currentState) {
    ...
    case STATE_INIT:
      if (!bothAtBottom) {
        // 策略1：如果在上限位，下一次通電一定往下
        if (anyAtTop) {
          moveDown();
          if (syncTimeout == 0) {
            syncTimeout = millis();
            Serial.println(F("[INIT] 從上限位開始下降..."));
          }
        }
        // 策略2：如果在中間位置，先往上到頂再下來（確保方向）
        else if (!anyAtBottom && !anyAtTop) {
          if (syncTimeout == 0) {
            syncTimeout = millis();
            Serial.println(F("[INIT] 中間位置 - 先往上確認方向..."));
          }
          // 先往上移動
          if (!lastMotorDirectionWasUp || !motorIsRunning) {
            moveUp();
          }
          // 如果已經碰到上限位，開始往下
          if (anyAtTop) {
            Serial.println(F("[INIT] 到達上限位 - 開始下降..."));
            stopAllPWM();
            delay(100);
            moveDown();
          }
        ...
        // 超時保護
        if (syncTimeout != 0 && millis() - syncTimeout > SYNC_TIMEOUT_MS) {
...
void setup() {
  ...
  Serial.println(F("===================================="));
  Serial.println(F("  雙致動器智能控制系統 v7.1"));
  ...
  Serial.println(F("\n【v7.1 改進項目】"));
  Serial.println(F("✓ 修正限位開關去抖動邏輯"));
  Serial.println(F("✓ 新增馬達方向記憶追蹤"));
  Serial.println(F("✓ 改進初始化流程（確保方向）"));
  Serial.println(F("✓ 優化限位檢查（任何狀態立即停止）"));
  Serial.println(F("✓ 序列埠顯示磁簧開關觸發狀態"));
  Serial.println(F("✓ 修正速度閾值灰色地帶（12-15 km/h）"));
  Serial.println(F("=====================================\n"));
...
void loop() {
  ...
  if(millis() - lastPrintTime > 1000) {
    ...
    if(SINGLE_ACTUATOR_TEST) {
      // 單致動器模式
      Serial.print(F("磁上:"));
      Serial.print(actuator1.upLimitPressed ? F("✓") : F("○"));
      Serial.print(F(" 磁下:"));
      Serial.print(actuator1.downLimitPressed ? F("✓") : F("○"));
    } else {
      // 雙致動器模式
      Serial.print(F("L上:"));
      Serial.print(actuator1.upLimitPressed ? F("✓") : F("○"));
      ...
      Serial.print(actuator1.downLimitPressed ? F("✓") : F("○"));
      Serial.print(F(" R上:"));
      Serial.print(actuator2.upLimitPressed ? F("✓") : F("○"));
      Serial.print(F(" R下:"));
      Serial.print(actuator2.downLimitPressed ? F("✓") : F("○"));
    }
    Serial.print(F("]"));
    ...
    Serial.print(F(" |馬達:"));
    Serial.print(motorIsRunning ? F("ON") : F("OFF"));
    if(motorIsRunning) {
      Serial.print(F("("));
      Serial.print(lastMotorDirectionWasUp ? F("↑") : F("↓"));
      Serial.print(F(")"));
    }
